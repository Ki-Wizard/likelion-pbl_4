...
- `Map<String, Role>`로 이름 기반 중복 확인과 검색 성능 개선
...
- 등록된 파트 목록 조회
- 파트별 멤버 필터링
- `MemberService`로 멤버 관리 로직과 콘솔 입출력 로직 분리
...
```bash
javac -encoding UTF-8 -Xlint:unchecked -d out src/class4/policy/*.java src/class4/role/*.java src/class4/service/*.java src/class4/package1/*.java src/class4/package2/*.java
```
...
- [x] `Map`을 사용하여 이름 기반 검색 성능 개선
- [x] 제네릭(`List<Role>`, `Map<String, List<Role>>`) 사용
- [x] 멤버 등록 시 중복 확인
- [x] 이름으로 멤버 검색
- [x] 파트별 멤버 필터링
- [x] `MemberService`로 역할 분리
